...
public final class MedicineResponseDTO {
    @Schema(name = "MedicineInfoResponse", description = "약 정보 응답 DTO")
    public record Info(
            @Schema(description = "약 UUID") String uuid,
            ...
            @Schema(description = "복용 시간") Set<IntakeTime> intakeTime,
    ...
    public record IntakeHistory(
            ...
            @Schema(description = "복용 상태") String status) {}

    @Schema(name = "MedicineDetailedInfoResponse", description = "약 복약 요약 응답")
    public record InfoWithHistory(Info medicine, List<IntakeHistory> history) {}
}
